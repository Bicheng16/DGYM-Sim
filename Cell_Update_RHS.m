...
function [dummy, prop] = Cell_Update_RHS(Target_Nodes, dummy, init, fluid, res, rockfluid, prop)
    ...
    prop = Properties_Fluids(next, prop, fluid, init, dummy, Target_Nodes, inherit, inherit_loc); 
    ...
    prop = Gas_Adsorption_Cals(next, prop, init, res, fluid, dummy, Target_Nodes, inherit, inherit_loc, false);
 
    % Gas slippage apparent permeability multiplier
    prop = Gas_Slippage_Cals(next, prop, init, res, fluid, dummy, Target_Nodes, inherit, inherit_loc, false);
     
    % General Fickian diffusion
    [prop] = Gas_GFick_Cals(next, prop, init, res, fluid, dummy, Target_Nodes, inherit, inherit_loc, false);
